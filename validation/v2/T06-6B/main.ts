import { runTitleModeTests } from "./titleModes.test";
...
declare global {
  interface Window {
    __T06_6B_VALIDATION__: ValidationState;
  }
}
...
const render = (state: ValidationState) => {
  window.__T06_6B_VALIDATION__ = state;
...
const run = async () => {
  ...
  const checks = [...await runTitleModeTests()];
