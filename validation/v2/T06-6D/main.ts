...
import { runT06_6DTests } from "./stageSessionLifecycle.test";
...
declare global {
  interface Window {
    __T06_6D_VALIDATION__: ValidationState;
  }
}
...
const render = (state: ValidationState) => {
  window.__T06_6D_VALIDATION__ = state;
...
const run = async () => {
  ...
  const checks = [...await runT06_6DTests()];
  checks.push(Object.freeze({
    name: "ブラウザconsole／unhandled rejection監視",
    ok: browserIssues.length === 0,
    detail: browserIssues.join(" | ") || "warning/error/unhandled rejection 0件"
  }));
