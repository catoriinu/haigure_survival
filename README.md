...
- ルーレット中のビット仕様は次のとおり。
  ...
  - 各ラウンドのビット数は `1..max(1, floor(参加キャラ総数 / 2))` で再抽選する。
  - 停止位置は離散スロットで、必ずいずれかのキャラの背後に停止する。
  - 回転時間は 10 秒（加速 1 秒、巡航 4 秒、減速 5 秒）。
  - 停止 1 秒後に停止先キャラへ発射判定し、停止先が洗脳済みキャラなら発射しない。
- 光線命中したキャラは公開処刑と同様の命中演出を経て `brainwash-complete-haigure`（画像名 `bw-complete-pose`）へ遷移する。
- 演出完了 1 秒後に次ラウンドのビット抽選を行い、全員洗脳されるまで繰り返す。
- ルーレット中の操作は次のとおり。
  - `Enter`: タイトルへ戻る
  - `R`: Undo（現在ラウンドの抽選直前状態へ巻き戻し。複数段対応、Redoなし、フェード演出あり）
