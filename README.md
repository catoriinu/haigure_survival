...
- タイトル画面で `public/stage/arena_roulette.json` を選択すると、ルーレット専用モードで開始する。
- タイトル設定は `NPC初期人数` 以外を無効化し、開始時の実行設定も `NPC初期人数` 以外は反映しない。
- 開始直後に参加キャラ全員を円陣配置し、全員 `evade` 状態で中央向き・移動不可になる。
- ルーレット中のビット仕様は次のとおり。
  - 黒ビットのみ出現する（赤ビットは出現しない）。
  - 各ラウンドのビット数は `1..max(1, floor(参加キャラ総数 / 2))` を基準に抽選する。
  - 誰にも命中しないラウンドが出たら、その回数を次回以降の最小値/最大値に加算する（上限は参加キャラ総数）。
  - 停止位置は離散スロットで、必ずいずれかのキャラの背後に停止する。
  - 出現/消滅演出は通常ステージのビット演出と同じ処理を使う。
- 回転はビット出現演出が完了してから開始する。
  - 回転開始 9 秒後に発射する。
  - 回転プロファイルは「加速 1 秒 → 減速開始時刻を 4〜5 秒でランダム化 → スリップ区間（長めに滑る）→ 最終停止」で、停止位置の読みづらさを優先した演出になっている。
  - 停止先が洗脳済みキャラのビットは発射しない。
- 光線命中したキャラは公開処刑と同様の命中演出を経て `brainwash-complete-haigure`（画像名 `bw-complete-pose`）へ遷移する。
- 光線は命中した時点で減衰し、対象を突き抜けない。
- 演出完了 1 秒後に次ラウンドのビット抽選を行い、全員洗脳されるまで繰り返す。
- 左上HUDに `未洗脳者数` `ルーレット回数` を表示し、プレイヤー洗脳後は `生存回数` も表示する。
- 回転中は `FlyingObject.mp3` を再生し、開始〜スリップ中は一定音量、最終停止区間のみ減衰して停止する。
- ルーレット中の操作は次のとおり。
  - `Enter`: タイトルへ戻る
  - `R`: 1回分やりなおす（現在ラウンドの抽選直前状態へ巻き戻し。複数段対応、Redoなし、フェード演出あり。ラウンド回数/無命中補正/洗脳状態も巻き戻す）
