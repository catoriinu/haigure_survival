...
- Blenderは457 Object／432 Meshとし、Export Collection配下の434 ObjectだけをGLBへ出力する。
- 出力内訳は`VIS_*`274件、`COL_*`147件、`NAV_*`8件、`META_Stage`1件、`MRK_*`1件、`VOL_*`2件、`BND_Stage`1件とする。
- 制作用ガイド23件はExport Collection外に保持し、GLBへ出力しない。
- 主玄関、北側校舎北口、北側校舎南口・渡り廊下西側、体育館校庭側は、総高0.30m、各蹴上0.15m、各踏面1.00mの表示2段を共通断面とする。移動衝突は表示段と分離し、地面Z＝-0.30mから床Z＝0.00mへ続く単一の`COL_*Ramp`とする。
...
- `BND_Stage`のBlender Z範囲は-0.5～4.6mとし、中間踊り場上面Z＝2.4mに床余裕0.05mと最大視点高2.0mを加えたZ＝4.45mを内包する。
- 事前ベイク後は、主玄関スポーンから学校1階、3階段踊り場、体育館、体育倉庫、舞台、北側屋外、渡り廊下側面、男女トイレ個室を含む代表15経路が要求終点へ到達することを検査する。部分経路を成功扱いせず、要求終点との誤差も検査する。
- 現行GLBは774,208 bytes・SHA-256 `C8799EE3C62438795E8153F9C006E66DD089EBCAAD7924DA1909F7E857F79BB8`、NavMeshは158,784 bytes・SHA-256 `571C6D488D174562FD6C3FE51E4856124D8F61CB0AE0B09D0A2FFD014A907744`である。同じ保存済み`.blend`からのGLB再出力と同じGLBからのNavMesh再ベイクで、各SHA-256が一致しなければならない。
