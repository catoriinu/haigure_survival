...
学校窓はB02で方針を確定し、B03、T04、T05、T06で実装・統合する。T01～T03時点の共通ローダーには、移動体用衝突と光線遮蔽用衝突の分類処理はまだ存在しないため、本節のゲーム側分類はT05で実装する。

| メッシュ | 役割 | 表示 | プレイヤー・NPC・ビット | 光線 |
|---|---|---|---|---|
| `VIS_WindowFrame_*` | 低ポリ窓枠 | 表示 | 衝突判定を持たない | 衝突判定を持たない |
| `VIS_WindowGlass_*` | 薄い透明ガラス | 表示 | 衝突判定を持たない | 透過 |
| `COL_ActorOnly_Window_*` | 窓開口を塞ぐ移動体専用不可視衝突 | 非表示 | 通過不可 | 判定対象外 |

- 壁は窓位置を実際に開口し、開口周囲の上下左右を低ポリ壁として構成する。大量のBoolean適用を前提にせず、分割壁で安定した形状を作る。
- ガラスは1枚面または極薄板とし、全窓で同じ透明ガラスマテリアルを共有する。画像テクスチャや多層透明面を必須としない。
- 透明度は実機で視認性と負荷を確認して確定する。初期候補は90～95%透明とし、99%透明も選択可能だが、窓枠を含めて窓の存在を識別できる状態を維持する。
- 同じ階・壁面のガラスは可能な範囲でまとめ、透明オブジェクト数、マテリアル数、重なり描画を抑える。
- `COL_ActorOnly_Window_*`は既存`COL_`規約に含まれるため、現行ローダーでもプレイヤー衝突は有効になる。T04は窓越しのNPCナビゲーションリンクを作らず、T05はこの接頭辞を移動体用衝突集合へ含め、光線遮蔽用集合から除外する。
- T06では、代表窓ごとにプレイヤー、NPC、ビットの通過不可、通常光線・固定光線・トラップ光線の透過、窓周囲の通常壁への着弾を検証する。
