...
- N選択（brainwash-complete-no-gun）
  ...
  - `銃なしに触れたら洗脳` がONのときは、移動封じの代わりに接触対象を4秒演出で洗脳する
...
- `銃なしに触れたら洗脳` がONのとき、`brainwash-complete-no-gun` 接触でも4秒演出（hit-b→足元からhit-a）後に同様にbrainwash-in-progress系へ遷移する
...
- brainwash-complete-no-gun
  ...
  - `銃なしに触れたら洗脳` がONのときは、移動封じを行わず接触対象を4秒演出で洗脳する
