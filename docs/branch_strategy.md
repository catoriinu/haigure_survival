...
2026-08-09、PR #64のT06-1、PR #65のB03-3D、PR #66の荒れ版家具干渉修正、PR #67のT06-2は`develop`の`3c5a248`までに統合済みであり、I2設計文書はPR #68で`develop=ceea8ec`へ統合済みである。B05はこのheadから専用branchで実装・検証を完了したが、独立レビューと`develop`統合は未実施である。後続のT06-3、T06-4は直列実行し、現在は開始しない。
...
- B05は`origin/develop=ceea8ec`から単独開始し、階別平面形状、全歩行可能Area、主要24目的地、階段、全階エレベーター乗場、放送卓の生成正本、`.blend`、GLB、監査、カタログhashを単一編集して実装・検証を完了した。人間用／Room Variant用／BIT用NavMeshは不変であり、独立レビューと`develop`統合を待つ。
- T06-3はB05が独立レビュー後に`develop`へ統合済みの最新headから単独開始し、B05の意味資産だけを読むミニマップを実装する。学校座標をTypeScriptへハードコードせず、学校バイナリを変更しない。
- T06-4はT06-3が独立レビュー後に`develop`へ統合済みの最新headから単独開始し、プレイヤー／NPC Mission、seed付き20秒scheduler、同行者、放送、公開処刑会場切替、HUDを統合してV3A機能ゲートを完了する。
...
- 現行順序は`I2・T06-2統合済み → B05実装・検証済み → B05独立レビュー・merge → T06-3 → T06-3独立レビュー・merge → T06-4 → I3 → T06-5 → T07 → v2リリース準備`とし、依存成果が`develop`へ入る前に後続ブランチを作成したり書込みを開始したりしない。
- `.blend`と`.glb`は同時に複数ブランチで編集しない。Blender資産は常に1ブランチ・1担当とする。
- 旧`codex/v2-post-pr66-roadmap` worktreeはdirty差分を保持したまま凍結する。T06-2用`codex/v2-t06-school-integration`はPR #67で完了済みであり、B05用`codex/v2-mission-locations`は最新`develop`から作成して実装・検証済みである。B05の独立レビュー・merge前にT06-3用branch／worktreeを作成しない。
