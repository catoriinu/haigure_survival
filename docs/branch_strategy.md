...
現在状態: T06-6DはPR #83で`develop=c8769e32`へ統合済み。P2文書は`codex/v2-final-polish-plan`で未コミット。P2はユーザー指定の人間確認OKを含めて完了、必須修正0件。T07の受入上の開始条件は成立した。開始時に未コミットのP2確定文書を引き継ぐ。[P2結果](plans/v2/P2/results.md)を参照する。
...
| P1 | `codex/v2-polish-wave-1-plan` |
| B06-1 | `codex/v2-school-structure-polish` |
| B06-2 | `codex/v2-elevator-building-integration` |
| B06-3 | `codex/v2-school-props-signage` |
...
| T06-6C | `codex/v2-title-game-over` |
| T06-6D | `codex/v2-static-school-resource-reuse` |
| P2 | `codex/v2-final-polish-plan` |
...
| I2 ミニマップ・Mission・Location・放送機能設計 | `codex/v2-map-mission-design` |
| B05 Mission・ミニマップ意味資産 | `codex/v2-mission-locations` |
| T06-3 ミニマップ | `codex/v2-minimap` |
| T06-4 Mission・放送Runtime | `codex/v2-missions` |
| P1 第1微修正整理 | `codex/v2-polish-wave-1-plan` |
| B06-1 学校構造仕上げ | `codex/v2-school-structure-polish` |
| B06-2 エレベーター建築統合 | `codex/v2-elevator-building-integration` |
| B06-3 小物・表札 | `codex/v2-school-props-signage` |
...
| T06-6C V1タイトル・ゲームオーバー | `codex/v2-title-game-over` |
| T06-6D 静的学校資源再利用 | `codex/v2-static-school-resource-reuse` |
| P2 最終微修正整理 | `codex/v2-final-polish-plan` |
| T07 性能・回帰・仕様書 | `codex/v2-t07-regression-docs` |

PR #50までの実行順は、B03-0とT04-2Aの並行Wave、B03-1、B03-2、T04-2B、T05-1、T05-2で完了した。その後、B03-3Aで学校拡張契約を確定し、B03-3BとT05-2V、B03-3C・T04-3A・T05-3のI0統合、B04とT05-4の並行Wave、T04-3B、通常版教室配置、通常テスト人口更新、B03-3D、T06-1、B03-3D追補、T06-2までPR #64～#67で完了した。I2設計文書はPR #68、B05はPR #69、T06-3はPR #70、T06-4はPR #71で`develop=8befad7`へ統合済みである。
...
- I2は相談専用タスクとしてT06-2前に先行完了した。階別Map、階段、エレベーター、Location、Mission、同行者、放送、公開処刑会場切替をユーザー承認で確定し、コード・資産は変更しない。
- B03-3D追補はPR #66で`develop`へ統合済みであり、学校生成正本、`.blend`、GLB、Room Variant NavMesh、生成器、監査器、カタログhashの編集を完了した。
- T06-2は開始候補、抽選方式、出現禁止Volume寸法、NPC／BIT出現Volume範囲の承認後、複数開始地点、動的出現禁止、NPC／BIT出現、時間増援、Player開始地点別NPC近傍優先を実装・検証し、PR #67で`develop=3c5a248`へ統合済みである。外周BIT飛行帯、外周BIT経路、外周スポーンは追加していない。
...
- P2はT06-6D後の文書専用仕分けタスクとする。必須修正は1問題1branchの`codex/fix-v2-<slug>`で閉じ、必須修正0件とP2代表受入完了の両方が成立するまでT07を開始しない。今回のコード・資産・テスト変更、commit、push、PR作成、merge、worktree削除は対象外。
- T07はP2後にAstraが単独で実施し、仕様不変のリファクタリング・軽量化・テスト整理を調査・実施する。1080p既定通常構成の60fps目標と、鏡面反射なしの99 NPC／50 BIT・120秒安定性stressを分けて最終回帰・仕様書同期を行う。学校バイナリと既知機能不具合の修正は混在させない。後発のユーザー指定によるP2-U01の起動案内修正はT07内の独立した実装単位として扱う。
- 現行順序は`T06-6D統合済み → P2完了・確定文書引継ぎ → T07 → v2リリース準備`とし、依存成果が`develop`へ入る前に後続ブランチを作成したり書込みを開始したりしない。
...
- 未仕分けの微修正を進行中タスクやT07へ便乗させること
- T06-6A～Dを並行してタイトル・保存・起動・ゲームセッション・学校Scene寿命を編集すること
