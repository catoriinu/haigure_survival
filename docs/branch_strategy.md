...
| B06 | `codex/v2-school-asset-polish` |
| T06-4P | `codex/v2-runtime-ui-polish` |
...
| B06 学校資産微修正 | `codex/v2-school-asset-polish` |
| T06-4P Runtime／UI微修正 | `codex/v2-runtime-ui-polish` |
...
2026-08-10、PR #64～#70は`develop=7f17e2d`までに統合済みであり、T06-4はPR #71で`develop=8befad7`へ統合済みとなった。後続は`P1 → B06 → T06-4P → I3 → T06-5 → I4 → T06-6A → T06-6B → P2 → T07`を直列実行する。
...
- I0、B04、T05-4、T04-3B、B03-3D、T06-1、T06-2、T06-3、B05、T06-4、B06、T06-4P、T06-5、T06-6A、T06-6B、T07はPull Request作成後、`develop`へマージする前に`docs/plans/v2/next_tasks_plan.md`の対応する重点動作確認ゲートと独立レビューを完了する。
...
- P1はT06-4後の文書専用仕分けタスクとする。B06はP1で承認された学校資産だけを単一担当で修正し、T06-4PはB06統合後の最終資産を入力としてRuntime／UIだけを修正する。両者は順序依存のため並行しない。
...
- T06-6AとT06-6Bはタイトル、設定schema、保存、起動snapshotの共有ファイルを使うため直列化する。T06-6Aが基本設定、T06-6Bが開始モードと機能ON／OFFを所有し、V3Cタイトル・設定ゲートを完了する。
- P2はT06-6B後の文書専用仕分けタスクとする。必須修正は1問題1branchの`codex/fix-v2-<slug>`で閉じ、必須修正0件になるまでT07を開始しない。
...
- 現行順序は`T06-4統合済み → P1 → B06 → T06-4P → I3 → T06-5 → I4 → T06-6A → T06-6B → P2 → T07 → v2リリース準備`とし、依存成果が`develop`へ入る前に後続ブランチを作成したり書込みを開始したりしない。
...
- T06-6AとT06-6Bを並行してタイトル・保存・起動処理を編集すること
