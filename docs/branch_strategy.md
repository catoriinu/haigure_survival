...
PR #50までの実行順は、B03-0とT04-2Aの並行Wave、B03-1、B03-2、T04-2B、T05-1、T05-2で完了した。以後は、境界契約を固定した後にB04とT05-2Vを並行し、両方を`develop`へ統合してからT06、T07を直列に実施する。
...
- B03-1とB03-2は同じ学校`.blend`とGLBを順番に編集する。T04-2B、T05-1、T05-2までは直列実行済みである。後続はB04とT05-2Vだけを並行し、両方の完了後にT06、T07を直列実行する。
...
- PR #50後のT05-2VはV1光線表示、演出資源再利用、`BND_WorldLimit`退出点での0.2秒非着弾フェードを担当し、学校バイナリ資産を編集しない。
- B04は学校`.blend`、生成正本、GLB、両NavMesh、カタログhashの単一担当として、塀外5.0mの歩道・道路、新規`BND_WorldLimit`、外周BIT飛行帯を追加する。外周へ人間用NavMeshまたは出現Volumeを追加しない。
- B04とT05-2Vは、`BND_WorldLimit`契約を先に固定し、別worktree・別ブランチで並行できる。B04だけが学校バイナリ資産を所有し、T05-2VはRuntimeコードと非学校fixtureだけを所有する。
...
- T06はB04の外周資産とT05-2Vの世界境界Runtimeを実学校へ接続し、BITが追跡・逃走・明示遷移中だけ外周へ一時進入でき、通常探索・待機・増援出現では外周を使用しないことを検証する。
