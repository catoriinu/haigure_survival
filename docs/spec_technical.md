...
V2のステージ実行契約は、新規作成予定の [V2ステージランタイム仕様書](./spec_stage_runtime_v2.md) を正本とする。本節は同仕様書へ移行するための先行規定である。
...
以下はv1.3.1／旧T02時点の実装を追跡するために残す。V2実装との不一致は本節と `docs/spec_stage_runtime_v2.md` を優先する。

## 1. 文書の位置付け

本書は、HAIGURE SURVIVAL v1.3.1の実装構成、主要データフロー、状態管理、保守上の制約をまとめる。ゲームとしての挙動は [ゲーム仕様書](./spec.md)、ステージデータ契約は [ステージJSON仕様書](./spec_stage_json.md) を参照すること。
...
読込時は数値範囲、真偽値、ステージID、画像・ボイスディレクトリ、処刑方式、割合合計を正規化する。現行v6と直前v5だけを受け入れ、それ以外は既定値へ戻す。正規化で値が変わった場合は修正済みデータを保存し直す。
