...
> タイトル画面右パネルの各要素の行間を、もう3px程度空けてください。
>
> 「ステージ」は幅を20～30px広げ、「プレイヤー開始地点」が少なくとも1行で表示できるようにしてください。
>
> 「ステージ」「人口」など、各設定枠のタイトル表示の下へ、もう3px程度の余白を追加してください。
...
- [x] wide右パネルの各設定行の上余白を3px増やす
- [x] ステージ幅とラベル列を広げ、「プレイヤー開始地点」を1行表示にする
- [x] 各設定枠の見出し下余白を3px増やす
- [x] 1920×1080／1280×720の実ブラウザとT06-6C buildで再検証する
- [x] 承認済み結果を計画へ同期し、対象差分だけをcommitする
...
- 1920×1080、1280×720、1024×600、800×600、640×480、480×360の通常／即時公開処刑をElectron fixtureで確認し、横overflow 0、font 12px以上、小画面操作高32px以上、全設定到達、連続resize時の値・mode・focus・version維持を確認した。wideはT06-6C以前と同等の操作密度を基準に、右パネル最大405px、ステージ330px、音量250pxとした。ステージのラベル列を118pxへ広げて「プレイヤー開始地点」を1行表示にし、wide右パネルの設定行上余白を5px、設定枠見出し下余白を7px、ステージ／音量の見出し下余白を11pxとした。1920×1080の通常モードは右パネル縦overflow 0であり、1280×720でもステージ、中央タイトル、右パネルの重なりと横overflowはない。
...
## 2026-09-12 右設定パネルの幅調整

### プロンプト

> タイトル画面の右側の設定ウィンドウについて、幅を20pxほど狭くしてほしいです。
> 文字を折り返さないように、バーやスペースだけを上手く詰めてください。

### ステップ

- [x] 現在の右パネル幅405pxとラベル・バーの配置を確認した。
- [x] wide表示の右パネルだけ20px縮小し、文字サイズを維持して横間隔と操作部品の幅を調整した。
- [x] 通常ゲームの実画面で幅・折り返し・操作と素材読込を確認した。

### 結果

- 1920×1080と1280×720のwide表示で右パネル幅385px（変更前405px）を実測。全18ラベルが1行で、内部の横overflowは0件。文字サイズ13pxと縦間隔は維持した。
- スライダーのキー操作で視点高さ1.15→1.20を確認し、1.15へ戻した。公開処刑設定でも横overflowがないことを確認し、サバイバルモードへ戻した。
- 通常ゲームの正本素材表示、Character／VOICE選択肢、音量各5・非MUTE、console警告／エラー0件を確認した。BGM／SE／VOICE／Character代表素材はHTTP 200かつaudio/mpeg・audio/wav・image/pngで応答した。
- 正本作業木のViteをポート5175で再起動し、起動元をプロセスコマンドラインで照合した。確認URL: http://127.0.0.1:5175/ 。
- 検証画像: `verification-images/title-panel-width/after.png`（ディレクトリ単位でGit除外済み）。
- CSSと本計画のみ変更。差分・UTF-8 BOMなしを確認。ユーザー指示により、今回のCSS変更と記録を作業ブランチへローカルコミットする。

### 追加指示：左右の内側余白を統一

> 右側の設定ウィンドウ内の右側にマージンがまだありますね。左側のマージンと同じになるよう、右に詰めてください

- [x] 原因の`scrollbar-gutter: stable`を確認し、wide表示のみ`auto`へ変更した。
- [x] 1920×1080でパネル幅385pxを維持し、内側の設定枠まで左右とも7px（枠線1px＋余白6px）、横overflow 0件を実測した。
- スクロール不要時の右側15pxの予約領域を解消した。高さが足りない場合は従来どおり縦スクロールできる。
- 検証画像: `verification-images/title-panel-width/after-margin.png`（Git除外済み）。確認サーバーは前項と同じ正本作業木・ポート5175・正本素材を継続利用。幅調整と併せてローカルコミットする。
