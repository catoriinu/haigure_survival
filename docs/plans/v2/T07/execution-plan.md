...
現在状態: PR #84作成後のWeb追加検証・独立レビューと実プレイ影響報告を経て、ユーザーが既知9項目を許容した。T07／V4はユーザー判断による受入OK。採否の正本は [最終受入記録](accepted-known-issues.md)、今回の作業は [マージ準備計画](merge-preparation-plan.md) を参照する。以下の初回計測FAIL・未確定は履歴として維持する。
...
下記の当初依頼にある未解消項目の修正前提は、現行WebのT07-K01～K09について許容済みの既知問題へ置換した。過去の測定結果をPASSへ書き換えず、Web限定と再開時間測定不要の後発指定を維持する。
...
- [x] PR作成後のWeb追加検証・独立レビューを行い、既知9項目のユーザー許容によりT07／V4受入を完了。
...
開始条件確認、計測基盤の独立検証、before／after正式計測、候補の最終採否、再開始保持観測と補助診断を完了した。P2-U01、Mission HUD、空間query membership、13 suite共通runner、B06構造上限追随を採用し、他候補は契約または寄与の根拠に従って非採用とした。その後HUD・体力・拘束・即時公開処刑リプレイを整備し、PR #84を作成した。最終Web検証後、移動拘束・保持量・正式比較等の未確定と機能・性能の未達を既知9項目としてユーザーが許容し、T07／V4受入を完了した。数値条件と回帰項目は[担当計画](plan.md)、各実測は[結果](results.md)、最新の採否は[最終受入記録](accepted-known-issues.md)へ集約する。
...
`final-functional-acceptance/regression/`の13 suite中、B05、T04学校統合、T05、T05 NPC操作、T06、T06-2、T06-3、T06-4、T06-6A／6B／6C／6Dの12 suiteがPASSした。T04学校統合は82/82、T04総合は最終116/118で、残る2件は上記の資産問題である。FAILの実測は保持し、後発のユーザー判断によりT07-K02／K03として許容して受入を完了した。
...
固定4200frame参考pairはbeforeのruntime診断未達、afterの終盤Pointer Lock解除に加え、歴史的I3条件との素材・音量・機能差があるため比較未確定とした。現行値はbeforeのwork p95／p99=30.3／35.6ms、旧total p95／p99=33.9／39.2msに対し、afterは29.1／34.1ms、33.7／38.5ms。採取実施は完了としても合格証拠にはしない。当初残した学校資産・移動拘束・保持・媒体・通常性能の修正と再評価は、後発のユーザー判断で許容済み既知問題へ置換した。正式V4はPR作成後のWeb追加検証・独立レビューと人間判断により受入を完了し、PR #84のマージを別工程として残す。
