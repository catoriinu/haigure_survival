...
- 通過窓は有効開口1.08m角以上を必須とし、B03では施工余裕を含む1.20m×1.20mを設計目標とする。
...
|---|---|---:|---|---|
| W02-R1 | B02の18窓帯／階を壁面候補として再配置。各室外壁、中庭側廊下、端部へ配分し、体育館は北・東・西に各3窓。南面は窓なし | 校舎72＋体育館9＝81窓。開33、閉48、開放率40.7% | 現Blockoutと整合し、全室・廊下と体育館3面を覆いながらObject数を抑えられる | 大型特別教室では窓帯間隔がやや広い |
...
採用: **W02-R1（体育館南面窓なし修正版）**。校舎は各階18窓、全72窓とし、主要居室、廊下、外周、中庭側を漏れなく覆う。体育館は北・東・西の上部に各3窓を置き、南面（舞台側）には窓を一切設けない。全81窓のうち33窓を開放し、開放率40.7%とする。
...
各開放窓は有効開口1.20m×1.20mを目標とし、半径0.54m包絡を通す。閉鎖窓は`COL_ActorOnly_Window_*`、開放窓は`COL_HumanOnly_Window_*`と`bit_window` pairを持つ。
