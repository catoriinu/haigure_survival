...
## 2026-09-12 BIT出現の屋内50%配分

### プロンプト

> 案1「屋内全体の出現配分を50%にする」。これでお願いします。体育館も屋内に含まれてよいです。比率は後で調整しましょう。

> ここまで一回コミットしてください。（2026-09-12、透明深度修正の検証完了後）

### 対象と完了条件

BITの初期出現と時間増援について、屋内50%・屋外50%を1機ごとに抽選し、各分類の内部を既存のVolumeとNavMeshの交差面積比例で選ぶ。体育館も資産のindoor分類に従い屋内へ含める。比率は後から調整できる単一定数へ集約する。候補棄却後もその機の分類を維持し、既存の安全判定・禁止領域・開始地点除外・最小距離・試行上限を守る。

有効な出現領域に片方の分類しか存在しないステージでは、その分類だけを使い分類抽選の乱数を消費しない。生成候補が枯渇した場合は既存の上限後エラーを維持し、別分類への振替や生成延期は追加しない。Alert近傍追加生成、出現後の巡回・索敵、タイトルUI、学校資産は今回の対象に含めない。出現時の抽選比率と移動後の所在比率を区別して検証する。

### ステップ

- [x] 現行の出現・乱数・候補棄却と有効な出現分類の契約を確認する。
- [x] 屋内外別の面積samplerと、1機ごとに1回の分類抽選を初期・時間増援へ接続する。
- [x] 面積差、0.5境界、同分類での再試行、片側のみ、同seed再現の回帰と型検査を確認する。
- [x] 実学校の初期・時間増援を複数seedで測定し、安全・除外・間隔・分類別生成数を確認する。
- [x] 通常ゲームと関連ブラウザfixture、文字コード・差分を確認し、結果を記録する。

### 結果

`src/v2/bitSystem.ts`の`V2_BIT_INDOOR_SPAWN_PROBABILITY = 0.5`へ比率を集約し、実出現Volumeのzoneが持つspaceKindで分類した。分類は1機ごと・候補再試行の外で選ぶ。分類内は面積比例と既存安全判定を維持した。資産の施設名・学校固有IDはRuntimeへ追加していない。

T06-2へ面積1:9の合成fixtureを追加し、0.5境界、初期と時間増援、分類を維持した再試行、候補全滅時の失敗、屋内のみ／屋外のみ、同seed再現を確認した。実学校検証の追加に伴い、T06-2のCSPへ通常ゲーム・T05と同じ`wasm-unsafe-eval`を追加し、実Recast NavMeshを読み込めるようにした。

実学校GLB・NavMeshを使用し、荒れ度5・ステージseed0で部屋と扉を固定して、出現乱数18seedを検証した。初期30機×18は屋内269／屋外271（49.8%屋内）、通常初期1機×18は屋内9／屋外9、時間増援29機×18は屋内260／屋外262（49.8%屋内）。集計1080機、開始地点11種類、同seedの再生成60機を含む1140機すべてで安全包絡・禁止領域・開始地点除外・最小間隔を満たした。最小間隔は0.2173 world units（設定0.2）、上限30と同seed位置一致もPASS。計測中は敵対行動を停止して移動後の所在と混同せず、公開APIで出現演出を完了して座標を取得した。全室normal条件での先行測定も1080機・安全検査1140機がPASSしている。地形・扉を全seedで変えた測定や、実プレイ中の所在比率の測定ではない。

最終ブラウザT06-2は31/31、T05は338/338 PASS、両fixtureのBabylon Logger・console warning/errorは0件。`typecheck:v2`、`typecheck:t05`、`typecheck:t06-2`、`build:renderer`（依存監査・配布監査を含む）がPASS。buildには既存のBabylon.js 4MB超chunk警告がある。差分・文字コード・ローカル絶対パス混入も確認した。

正本作業木の通常確認サーバーを同じ5175で再起動し、プロセスの起動元を照合した。通常ゲーム`http://localhost:5175/`、出現検証`http://localhost:5175/validation/v2/T06-2/index.html`、既存戦闘検証`http://127.0.0.1:5180/`で確認できる。正本の音声・Character素材を使い、代表BGM/SE/VOICE/画像のHTTP 200とaudio/mpeg・audio/wav・image/pngを確認した。通常タイトルのカタログは非空で各音量5、クリック開始後にNPC50人とBIT1→3体を確認した。素材読込エラーはないが、アプリ内ブラウザのPointer Lockは既存のWrongDocumentErrorで失敗し、マウス視点操作は未確認。画像はGit除外済み`verification-images/bit-indoor-spawn/normal-game.png`へ保存した。

2026-09-12の追加指示により、この屋内出現配分と透明深度修正を1件のローカルcommit対象とする。別タスクで進行中のNPC巡回差分は含めず、pushは行わない。
