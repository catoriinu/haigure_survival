...
2026-09-06 PR #83対応依頼：プルリクにコメントが付いたので内容の妥当性を確認して修正してください。その後プッシュしてインラインコメントに返信まで行ってください。
...
- [x] PR #83の再試行クリック接続に関する指摘を確認し、Canvasへアプリ寿命で接続して終了時に解除する
- [x] 初回ロード失敗を注入する専用Electronで設定操作と明示再試行を分離し、現在設定で復旧することを確認する
