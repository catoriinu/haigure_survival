...
- NavMeshはRecast系を技術候補とし、学校で実行時生成時間と事前ベイク読込を比較してから正本形式を確定する。長期運用では事前ベイクしたバイナリを優先する。
...
- [ ] プレイヤースポーン、床高、衝突、ステージ境界、再配置を3D空間APIへ移行する
...
7. `refactor: プレイヤーを3Dステージ基盤へ移行`
8. `refactor: NPCの移動をNavMeshへ移行`
9. `refactor: ビットの移動をNavMeshへ移行`
10. `fix: 3Dステージ壁でビームと視界を遮蔽`
11. `refactor: V2のセル・JSONステージ依存を削除`
12. `docs: V2学校ステージ移行結果を更新`
