...
- [ ] B03の実窓で移動体専用衝突、視線、通常ビームの分類を技術検証する
- [x] Web・Electronで事前ベイクNavMeshの本番読込経路を技術検証する
- [ ] 重複床で別階へ誤投影しない高さ付きNavMesh問い合わせと、連続階段・非連続`LNK_*`接続を技術検証する
- [ ] NPC・ビット共通経路が学校の扉・階段を使って複数階の標的へ完全経路で迂回し、経路がない場合は壁へ直進せず停止する回帰を追加する
