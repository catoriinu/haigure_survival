...
| T04-2A 高さ付きNav基盤 | 未着手 | `codex/v2-t04-height-nav-core` | GPT-5.6 Sol | Ultra | `navigationWorld`、`navigationAgent`、重複床・階段・`LNK_*`専用fixture。学校バイナリは変更しない |
| T04-2B 学校複数階統合 | 未着手 | `codex/v2-t04-school-multifloor` | GPT-5.6 Sol | Ultra | B03最終GLBからのNavMesh、カタログハッシュ、NPC複数階、実窓分類、実学校回帰 |
...
- [ ] `docs/plan.md`、`docs/plans/v2/next_tasks_plan.md`、本計画、`docs/branch_strategy.md`、3D資産・Runtime仕様書を読む
...
- [ ] T04-2Aで重複床へ誤投影しない高さ付きNavMesh問い合わせと、連続階段・非連続`LNK_*`接続を専用fixtureで技術検証する
- [ ] T04-2BでNPC共通経路が学校の扉・階段を使って複数階の標的へ完全経路で迂回し、経路がない場合は壁へ直進せず停止する回帰を追加する。ビット固有の複数階戦闘はT05へ残す
...
- 舞台階段の形状ディテールと同色境界はB03へ残す。重複床を扱う共通高さ付きNavMesh問い合わせとNPCの複数階移動はT04後続、ビット固有の飛行姿勢・壁余裕と戦闘統合はT05で実装する。
...
- T04の確定範囲である学校3D空間基盤、事前ベイクNavMesh、NPC・ビットの1階迂回、通常索敵、アラート共有、通常ビーム、JSON・セル実行経路廃止を完了した。固定／トラップ／動的ビーム、集合・ステージ警報ギミック、ビット固有の複数階戦闘はT05へ残し、共通高さ付きNavMesh問い合わせとNPCの複数階回帰はT04後続へ明記した。
- 現在の学校NavMeshは1階と3階段の中間踊り場までを連続面として含む。B02追加テストで北西階段はプレイヤー用Colliderにより2階、3階、4階、屋上まで接続されたが、2～4階の一般床、北西上階階段、屋上はNPC・ビット用NavMeshから明示的に除外した。T04実ブラウザでは各階の一般床・階段と屋上の7代表点がすべて投影不能・経路なしとなる40/40項目PASSを確認した。共通の高さ付きNavMesh問い合わせとNPCの複数階回帰はT04の後続項目、ビット固有の飛行姿勢・壁余裕と戦闘統合はT05の項目として分離する。
