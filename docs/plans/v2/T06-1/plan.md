...
実装・検証中。修正前HEADの固定distをseed `20260725`、校庭視点、1920×1080、99 NPC（初期洗脳66）／50 BITでcold 600＋steady 3600 frame計測した。steadyのframe workは平均29.089ms、p95 45.5ms、p99 120.6ms、最大153.9ms、主要section平均はNPC 16.501ms、BIT 4.228ms、render 5.334msだった。draw call平均333.103、active mesh平均426.472、total mesh平均1645.566、heap平均309.503MB／最大533.198MBで、console／load／render process消失／unresponsiveは0件だった。計測用runnerの標準出力pipe切断で一度だけ`EPIPE`が発生したため、通常ゲーム由来でないことと該当process残留0件を確認し、外部JSONだけへ書く親process非依存方式で全4200 frameを再取得した。
...
ゲーム中の「NPC内訳」について、「現在の人数」に表示を修正し、プレイヤー自身もカウントに含めてください。

追加要件：
- プレイヤーが未洗脳（通常か逃走中）の時点では、「現在の人数 生存者n人 洗脳済みn人」
- プレイヤーが洗脳済み（hit-a以降になったとき。はじめから洗脳済みの場合も含む）の時点では「現在の人数 未洗脳者n人 ハイグレ人間n人」に見出しも切り替える。

#### ステップ

- [x] 通常ゲームの操作案内内の人数表示と既存NPC状態分類を確認。
- [x] normal／evadeのプレイヤーを未洗脳側へ、それ以外を洗脳側へ1人加算し、同じ条件で見出しを切り替えた。
- [x] V2依存監査と型検査を確認。
- [x] 通常開始・洗脳済み開始の実画面を確認して結果を記録。

#### 結果

通常HUDの2群の合計をNPC数＋プレイヤー1人に変更した。BIT数と開発用のNPC詳細診断は既存どおり。追加指示「ここまでコミットして」により、人数表示・操作説明枠・本記録をローカルコミットする。pushは行わない。
通常開始の実画面で「現在の人数 生存者41人 洗脳済み10人」（NPC50人＋Player1人）を確認。実装の表示処理に全9状態を入力し、normal／evadeは41＋10人、hit-a以降7状態は40＋11人と対応する見出しになることを確認した。正本素材のBGM・SE・VOICE・Character画像はHTTP 200と音声／画像Content-Typeを確認。確認サーバーは正本作業木の5175番で再起動して維持する。

内蔵ブラウザでは開始時にPointer LockのWrongDocumentErrorを検出した。素材読込エラーは検出していない。マウス視点操作は未確認として区別する。

洗脳済み開始でも「現在の人数 未洗脳者40人 ハイグレ人間11人」を実画面で確認し、プレイヤーを含む合計51人と見出しの切り替えを確認した。

#### 操作説明枠の固定幅（2026-09-13）

追加プロンプト：
「ついでに、この操作説明の枠、右側に無駄にマージンが大きくとられてると思います。必要最小限まで狭くしてください。できれば想定されうる最長の文字長分の幅で固定してほしいです」
「操作説明の文章も含めての固定幅にしてくださいね」

- [x] 操作説明の全行、通常／洗脳済みの見出し、NPC上限99人＋Player1人の人数桁数を照合。
- [x] 操作説明の全行を比較し、最長人数行の全角17文字・半角7文字・24文字分の字間と枠余白をCSS幅へ指定した。内側余白は8px。狭い画面は既存の画面幅上限に従う。
- [x] 実画面で通常／洗脳済みの幅一致と文字の収まりを確認。

通常／洗脳済みの両表示は1280×720で外枠296.96875pxに一致し、横方向のはみ出しなし。検証用の洗脳済み開始設定は元のOFFへ復元。文字列に合わせたCSS幅の調整のみのため、型検査やゲーム全体buildは再実行しない。
