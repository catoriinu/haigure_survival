...
- 本節より下に記録したT02の実装・検証結果は、当時の完了履歴として保持する。
- ユーザー指示により、V2はJSON文字マップ、`GridLayout`、`FloorCell`、セルナビゲーションを廃止し、GLBの3D形状・マーカー・ボリュームとNavMeshへ移行する。
- T02で導入した`procedural-grid`、GLB用平面グリッド、既存8ステージv2 JSON、JSON共通ローダーはT04でV2実行経路とリポジトリから削除済みである。後方互換アダプターは作成しない。
- 既存8ステージは一旦V2対象外とし、将来それぞれをGLB・NavMesh化して学校と同じ3D資産契約で再導入する。
- 現行T02検証は、学校GLB／NavMeshのハッシュ、作者Nodeの厳格分類、マーカー・ボリューム・境界、Actor／ビーム／視線の遮蔽集合、NavMesh投影、破棄・再読込を直接確認する14項目へ再割当した。ステージJSON、`GridLayout`、`FloorCell`、旧共通ローダーは読み込まない。
- 本節より下の45項目検証などは、旧方式を完成させた当時の履歴であり、現行V2の実行仕様ではない。
