...
> 注記: 以下のプロンプトと旧結果は2026-07-18時点の実装履歴である。現行V2はJSON fixture、旧`StageContext`、既存8ステージ回帰を使用せず、学校GLB、`StageSpatialContext`、NavMeshを正本とする。
...
### 2026-07-19 学校3D実行基盤への移行

- T03の重力、接地、段差、連続斜面、階段ランプ移動は維持し、実運用接続を`src/v2/playerController.ts`へ移した。
- T03検証は平面コースをBabylon MeshBuilderで直接生成し、T01 GLBも直接ロードする。旧JSON→GLB→JSON切替は平面Mesh→GLB→平面Meshの資源切替へ置き換えた。
- 斜面中腹での入力解除、停止、逆方向入力、昇降速度、水平ドリフトを回帰し、後続の学校手動テスト回帰を含む全24項目がPASSした。プレイヤーとカメラは表示段ではなく`COL_StairRamp_*`上を滑らかに追従する。
- 学校3階段で踊り場直前の減速を実測したところ、修正前は平地比約0.44、接線移動だけでは約0.76だった。接地用liftのX/Zを固定し、要求水平変位と支持面高差から斜面接線の3D変位を作る修正後は、北西・北東・南西すべてactual/requested=`1.000000`、T03の踊り場直前速度比`1.006`、終端前後の負のY変位0frame、入力解除後drift 0となった。
- 境界外移動の巻き戻しでは`resync()`を再実行せず、フレーム開始時の足元、接地、垂直速度、支持面高、支持Mesh、支持面法線をSnapshot復元する。spawn時の再同期は維持し、再同期の垂直衝突前後でX/Zを固定する回帰も24項目へ含めた。
- T03検証と通常V2入口はステージJSON、`GridLayout`、`FloorCell`、セル座標変換を読み込まない。旧8ステージの平面回帰は過去の完了履歴として残し、現行V2対象からは外した。
