...
- Player洗脳後もG／N／H操作を継続可能にする。T06-6Cの後発契約に合わせ、通常playingのRはPlayer状態を問わず同じ設定snapshotから新seedで通常sessionを再生成する。開始地点、荒れvariant、Character割当、NPC／BIT配置は新seedで再抽選する。
- T06-6Cの後発契約に合わせ、通常playingのEnterはPlayer状態を問わず直接タイトルへ戻る。全human洗脳済みは3秒後に自動epilogueへ進む。
...
- 基準点は中央、左上、右上、左下、右下の5箇所とする。中央はタイトル／開始モード、左上は固定学校表示、右上は警告、左下は音量等の補助設定、右下は操作列と独立version表示へ割り当てる。T06-6Cの後発指示で「保存済み」ラベルは削除済みとし、再追加しない。
...
- 既定通常: NPC 50、初期洗脳済み20%、BIT最大30、荒れ度5、Mission ON、Alarm OFF。
...
2026-09-06 P2文書同期: PR #79～#83の統合とT06-6Cの後発結果を確認し、通常R／Enter、保存済みラベル削除の現行契約へ同期した。代表受入の既存証拠・AI確認・人間確認OKは[P2結果](../P2/results.md)を参照する。I4設計とT06-6A～Dは統合済み。P2の残確認はユーザー指定の人間確認OKとして閉じ、P2完了・必須修正0件とした。
