...
## 2026-09-12 同行NPCの入口停止の再調査

依頼: 「また別件ですけど、同行しているNPCがエレベーターに乗ってくれません。エレベーターに乗るように修正したはずですけど、これどういうことでしょうか。」

確認回答: エレベーター前までは来るが中に入らず、プレイヤーが車内の奥にいて空席がある状態でも止まったまま。

現行HEAD be42086を調査した。通常ゲームのユーザー画面で車内側から外のNPCが見える状態を確認し、console warning/errorは0件だった。通常入口には個別の乗車状態の診断表示がないため、そのNPCの予約・経路状態までは取得していない。

以前の同便優先・乗車予約修正は残っている。一方、updateFollowMovementは既知のPlayer便の目的階へ移動している場合も、移動後に現在のPlayerとの1m停止判定で経路を消す。追加した小回帰で0.25秒の一定目的地移動に経路選択が2回必要となり、保持を求めるassertが失敗した。これを独立した不具合として確認したが、ユーザー画面の停止の唯一の原因とまでは確定していない。調査用回帰を含む関連入口は88/89 PASS、失敗は追加した乗車接近経路の保持1件。

既存の通常ゲーム受入URLは、現在の保存設定（NPC99人）では初期人口不一致で起動を拒否した。50人前提の過去受入を、そのまま現在の99人設定の合格証拠にできない。ユーザーの保存設定は変更していない。

この時点では原因の質問に対する調査として、回帰と記録だけを追加した。Runtime変更、commit、pushは行っていない。修正案は、既知のPlayer便への追従中に通常の近距離停止で経路を破棄しないこと。通常の同行停止・拘束停止・乗車取消・到着後の追従を維持し、実際の入口停止は乗車状態を記録できる条件で追加検証する。
### 乗車接近中の停止判定の修正

> 「エレベーターの目的階へ向かっている途中」というのが、プレイヤーが乗り込んでいるエレベータの箱内に対してでも効くのであれば、それが原因かもしれません。修正をお願いします。エレベータに乗るときはその条件は無効にしてください。

対象: エレベーター追従・乗降状態中の通常同行停止判定。対象外: 定員、発車時間、陣営安全性、拘束優先、資産、探索予算。通常同行と乗車取消・到着後は既存の停止条件へ戻る。今回の修正は未コミットで検証・記録する。

- [x] 修正前の近距離経路破棄を回帰で再現する。
- [x] 乗車経路を近距離停止で破棄しないよう修正する。
- [x] 関連回帰・型検査と実乗車を確認し、結果と限界を記録する。
修正内容: updateFollowMovementで既知のPlayer便への追従またはNPC自身のエレベーター移動状態中は、隊列停止・停止済みフラグによる目的地破棄・移動後の1m停止を適用しない。拘束による停止は上位分岐のまま保持した。探索・経路計算の頻度や予算は増やしていない。

回帰実測: 呼出・予約・Player乗車中の3状態で、0.25秒間の経路選択は各1回。関連回帰89/89 PASS。通常同行の1m停止／1.2m再開、拘束停止・救助、取消後と到着後の復帰もPASS。typecheck:v2、typecheck:t05、Electron受入スクリプトの構文検査はPASS。

実ゲーム受入を保存設定と独立して再現できるよう、明示的なelevatorAcceptance URLだけ人口を固定し、elevatorNpcs=50（既定）または99を指定できるようにした。初期洗脳率20%、初期BIT1、増援10秒・最大25。通常URLと保存済みタイトル設定は変更しない。

50人・同行5人・seed20260821のElectron実学校受入は180秒でFAIL。4人（npc_10、npc_17、npc_20、npc_46）はPlayerと乗車し、4Fで降車した。最大予約込み6人・乗客5人、発車猶予3.9949秒、自律npc_7も乗車・到着。npc_12は乗車接近状態からwalkingへ戻って予約取消となり、最終的には4Fへ歩行到達した。5人全員の乗車成功とは扱わない。受入中のconsole／renderer／load／unresponsive等の記録は0件。終了時にはGPU終了メッセージが出た。

99人検証の制限: electron.cmd経由の試行はURLの&以降が分割され、実際には50人・同行5人で実行されたため99人の証拠から除外した。その試行も3人の乗車・降車までで全員受入はFAIL。直接exe起動では99人・同行1人の引数をプロセス情報で確認したが、結果出力を取得できていない。結果ファイル付きで再起動する操作は自動承認レビューに拒否された（検証プロセスの停止・再起動、詳細理由なし）。従って99人での実乗車と負荷は未確認。近距離停止判定の修正と関連回帰は完了しているが、同行全員の実乗車受入は未完了。commit・pushはしていない。

### 検証継続と予約保持（2026-09-12）

追加依頼: 「はい、検証を継続してください」。ユーザーが検証用Electronを手動終了した後も、新規起動が自動承認レビューで拒否されたため、Chromeの実学校回帰へ切り替えた。

実装上の原因: 空席数は予約込みで算出される。同行NPC自身が最後の席を予約すると空席0となり、同便追従の再判定が次便へ変わり、階段へ切り替わる場合がある。これはPlayerが先に乗った場合にも起きる。学校経路policyの同便判定で、当該NPC・出発階・目的階が一致する確保済み予約を空席とは別に扱う。定員や発車猶予は変更しない。

追加回帰は自分の予約で満員になった同行NPCの同便保持。未予約の満員便回避・次便追従の既存条件も維持する。Chrome実学校fixture 83/83 PASS（65.586秒）、typecheck:t04、typecheck:v2 PASS。T04検証用クラスに現行interfaceのsetBitAiSuspendedが欠落していたため、他の非対応操作と同じunexpected例外を実装して型検査を回復した。

追加訂正: 「NPCが先に乗っていて最後の一席に自分が乗るというケースは、実際にはほぼほぼ起こりません。なぜならNPCが自発的にエレベータに乗ってくれるのをみたことがないからです（本当は乗ってほしいんですが）。修正が簡単なら直してもいいですが、そのためだけに特殊な分岐を入れるなら既知の不具合にしてそのままでよいです。」

説明中の「自分」は同行NPC自身を指していたと補足した。特定の人数・乗車順の分岐ではなく、同便の確保済み予約を認める修正として継続。自律NPCは受入用の明示Missionでの乗車確認と、通常プレイでの自然利用頻度を分ける。

Chrome前面での99人・同行1人・seed20260821受入はPASS。同行npc_19の乗車・4F降車、明示Mission npc_7の乗車・到着、最大乗客2人、発車猶予3.986秒を確認した。読取時のゲーム内経過67.201秒、人口99・初期洗脳19。到着画面で通常Character素材と4F表示を確認した。検証途中の自動クリックによるPointer Lock要求がWrongDocumentErrorを1件記録しており、console 0件とは扱わない。乗車受入はPointer Lock不要で継続しPASS。Electron新規起動は拒否されたため、今回はChrome実測である。

最終結果: Chrome前面で50人・同行5人・seed20260821もPASS。npc_10、npc_12、npc_17、npc_20、npc_46の全員とPlayerが同便乗車、4Fで全員降車した。最大乗客／予約込みとも6人、発車猶予3.9999秒、自律npc_7も乗車・到着。読取時のゲーム内経過68.799秒。50人試行では新規console warning/errorなし（同一タブの過去Pointer Lockエラー1件のみ）。到着画面を確認した。

結果の範囲: 実学校Runtimeの受入用自動Player操作と実NPC移動で、99人・同行1人、50人・同行5人を確認。通常の人間による任意状況すべて、NPCの自然なエレベーター利用頻度、99人時の計算時間・経路待ち・移動途切れの変更前後比較は今回の結果では保証しない。元の巡回調整の性能受入は別記録の未確認を維持する。近距離停止と確保済み予約の取消に関する修正・回帰・実乗車検証を完了し、未コミットで保持した。

確認サーバー: 正本作業フォルダーの通常ゲーム http://localhost:5175/ 、実学校fixture http://localhost:5179/school-integration.html 。通常ゲームは正本音声・Character素材を使用。サーバーは継続起動。

コミット依頼（2026-09-12）: 「コミットメッセージを考えてコミットしてください。」関連8ファイルを「fix: 同行NPCのエレベーター乗車経路と確保済み予約を保持」の単位でローカルコミットする。pushは対象外。
